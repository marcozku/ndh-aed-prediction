...
```
預測值 = 星期均值 + 趨勢偏差×衰減 + 月份效應 + AI效應 + 天氣效應
```

**v3.0.50 更新**：改用加法效應模型，避免乘法疊加導致過高預測。

### 核心改進

| 改進項目 | 舊方法 | 新方法 (v3.0.50) |
|---------|--------|-----------------|
| 因子組合 | 乘法疊加 | 加法效應 |
| 星期均值 | 舊數據 (280 週一) | Post-COVID 校準 (270 週一) |
| 上限控制 | 無 | 180-320 硬上限 |
| 遠期預測 | 固定 | 趨勢衰減 (回歸均值) |

### 預測流程

1. 前端調用 `/api/ensemble-predict` API
2. API 執行 Python XGBoost 預測
3. 返回預測值和置信區間
4. 前端結合統計方法的元數據（因子分解顯示）
...
- 每 5 次訓練自動優化
- 每 50 筆新數據自動優化
- 從 `optimal_features.json` 動態加載最佳配置
...
**⚠️ v3.0.70 重要更新：AI 因子與其他因子去重**

為避免重複計算，AI 分析**不再包含**以下因素（已由系統自動處理）：

| 因素類型 | 處理方式 | AI 是否分析 |
|---------|---------|------------|
| 天氣（溫度/濕度/降雨） | Weather Factor 自動計算 | ❌ 不分析 |
| 公眾假期 | HK_PUBLIC_HOLIDAYS 定義 | ❌ 不分析 |
| 季節性流感 | fluSeasonFactor 自動計算 | ❌ 不分析 |
| 週末效應 | dowFactors 自動計算 | ❌ 不分析 |
| 月份效應 | monthFactors 自動計算 | ❌ 不分析 |
