/**
 * 導入歷史數據腳本
 * 將2015-2024年的歷史數據導入PostgreSQL數據庫
 */
...
function parseCSVData(csvText) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        // 修復日期格式錯誤（如 2024-10-23 寫成了 20244-10-23）
        let date = values[0].trim();
        date = date.replace(/^(\d{4})(\d{1})/, '$1'); // 修復多餘的年份數字
        ...
        if (date && !isNaN(attendance)) {
            data.push({
                ...
                notes: `從2015-2024年綜合分析報告導入的歷史數據`
            });
        }
    }
...
async function importHistoricalData() {
    ...
    try {
        ...
        if (dataToInsert.length > 0) {
            ...
            console.log(`📅 數據範圍: ${dates[0]} 至 ${dates[dates.length - 1]}`);
        }
